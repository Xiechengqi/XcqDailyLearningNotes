...
# 重新载入，每次执行完 firewall-cmd 都应该 reload 一次 ( 注意:这并不会中断已经建立的连接,如果打算中断,可以使用 --complete-reload 选项)
firewall-cmd --reload
...
# 删除规则编号或删除指定实际规则
delete num/rule
# 重置防火墙，该命令将禁用 ufw，并且删除所有已经定义的规则，所有规则将被重设为安装时的默认值，不过默认该命令会对已经设置的规则进行备份
sudo ufw reset
